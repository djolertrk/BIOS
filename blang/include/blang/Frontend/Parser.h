...
//===----------------------------------------------------------------------===//
//
// This file implements the parser for the bios language. It processes the Token
...
/// This is a simple recursive parser for the bios language. It produces a well
...
private:
  ...
  std::unique_ptr<ReturnExprAST> parseReturn() {
    ...
    // The 'return' takes an optional argument.
  ...
  ///   numberexpr ::= number
  std::unique_ptr<ExprAST> parseNumberExpr(NumberExprAST::NumberASTKind Kind) {
    auto loc = lexer.getLastLocation();

    if (Kind == NumberExprAST::Num_Float) {
      auto result =
          std::make_unique<FloatNumberExprAST>(std::move(loc), lexer.getValue());
      lexer.consume(tok_number);
      return std::move(result);
    } else if (Kind == NumberExprAST::Num_Float64) {
      auto result =
          std::make_unique<Float64NumberExprAST>(std::move(loc), lexer.getValue());
      lexer.consume(tok_number);
      return std::move(result);
    } else if (Kind == NumberExprAST::Num_Int) {
      auto result =
          std::make_unique<IntNumberExprAST>(std::move(loc), lexer.getValue());
      lexer.consume(tok_number);
      return std::move(result);
    } else if (Kind == NumberExprAST::Num_Int64) {
      auto result =
          std::make_unique<Int64NumberExprAST>(std::move(loc), lexer.getValue());
      lexer.consume(tok_number);
      return std::move(result);
    } else if (Kind == NumberExprAST::Num_Char) {
      auto result =
          std::make_unique<CharNumberExprAST>(std::move(loc), lexer.getValue());
      lexer.consume(tok_number);
      return std::move(result);
    }  else if (Kind == NumberExprAST::Num_Tensor) {
      auto result =
          std::make_unique<TensorNumberExprAST>(std::move(loc), lexer.getValue());
      lexer.consume(tok_number);
      return std::move(result);
    }

    llvm_unreachable("unknown ast number type");
  }
  ...
  std::unique_ptr<ExprAST> parseTensorLiteralExpr() {
    ...
    do {
      ...
      if (lexer.getCurToken() == '[') {
        ...
        values.push_back(parseNumberExpr(NumberExprAST::Num_Tensor));
      }
  ...
  std::unique_ptr<ExprAST> parseCallExpr(llvm::StringRef name,
    ...
    // It can be a builtin call to the 'print'.
    if (name == "print") {
      if (args.size() != 1)
  ...
  ///   ::= charexpr
  ///   ::= stringexpr
  std::unique_ptr<ExprAST> parsePrimary(ParsedExprType typeExpr = Type_Unknown) {
    switch (lexer.getCurToken()) {
    default:
    ...
    case tok_number: {
      NumberExprAST::NumberASTKind numType;

      switch (typeExpr) {
      default:
        // The default number is float64.
        numType = NumberExprAST::Num_Float64;
        break;
      case Type_Tensor:
        numType = NumberExprAST::Num_Tensor;
        break;
      case Type_Float:
        numType = NumberExprAST::Num_Float;
        break;
      case Type_Float64:
        numType = NumberExprAST::Num_Float64;
        break;
      case Type_Int:
        numType = NumberExprAST::Num_Int;
        break;
      case Type_Int64:
        numType = NumberExprAST::Num_Int64;
        break;
      case Type_Char:
        numType = NumberExprAST::Num_Char;
        break;
      case Type_String:
        numType = NumberExprAST::Num_String;
        break;
      }
      return parseNumberExpr(numType);
    }
    // Parse char type init with 'CHAR'.
    case tok_character: {
      auto loc = lexer.getLastLocation();
      auto result =
        std::make_unique<CharNumberExprAST>(std::move(loc),
                                            lexer.getCharValue());
      lexer.consume(tok_character);
      return std::move(result);
    }
    // Parse string type init with "some_string".
    case tok_string_primary: {
      auto loc = lexer.getLastLocation();
      auto result =
        std::make_unique<StringExprAST>(std::move(loc),
                                        lexer.getStringValue());
      lexer.consume(tok_string_primary);
      return std::move(result);
    }
    case '(':
  ...
  std::unique_ptr<ExprAST> parseExpression(ParsedExprType typeExpr = Type_Unknown) {
    auto lhs = parsePrimary(typeExpr);
    if (!lhs)
      return nullptr;
  ...
  std::unique_ptr<VarType> parseTensorType() {
    if (lexer.getCurToken() != '<')
      return parseError<VarType>("<", "to begin type");
    // Eat the '<'.
    lexer.getNextToken();

    std::unique_ptr<VarType> type = std::make_unique<TensorType>();

    while (lexer.getCurToken() == tok_number) {
      static_cast<TensorType*>(type.get())->getShape().push_back(lexer.getValue());
      lexer.getNextToken();
      if (lexer.getCurToken() == ',')
        lexer.getNextToken();
    }
  ...
  ///   type ::= num
  std::unique_ptr<VarType> parseFloatType() {
    std::unique_ptr<VarType> type = std::make_unique<FloatType>();

    return type;
  }

  /// Parse the float64 type.
  ///   type ::= num
  std::unique_ptr<VarType> parseFloat64Type() {
    std::unique_ptr<VarType> type = std::make_unique<Float64Type>();
  ...
  /// Parse the int type.
  ///   type ::= num
  std::unique_ptr<VarType> parseIntType() {
    std::unique_ptr<VarType> type = std::make_unique<IntType>();

    return type;
  }

  /// Parse the int64 type.
  ///   type ::= num
  std::unique_ptr<VarType> parseInt64Type() {
    std::unique_ptr<VarType> type = std::make_unique<Int64Type>();
  ...
  /// Parse the char type.
  ///   type ::= num | character
  std::unique_ptr<VarType> parseCharType() {
    std::unique_ptr<VarType> type = std::make_unique<CharType>();

    return type;
  }

  /// Parse the string type.
  ///   type ::= string
  std::unique_ptr<VarType> parseStringType() {
    std::unique_ptr<VarType> type = std::make_unique<StringType>();
  ...
  parseTypedDeclaration(llvm::StringRef typeName, bool requiresInitializer,
    ...
    auto shapeName = std::string(typeName);
    auto type = std::make_unique<TensorType>(shapeName);
  ...
  /// Parse a variable declaration, it starts with a type followed by
  /// an identifier. If the type is tensor, an optional shape specification
  /// before the optionally required initializer.
  ///
  ///   decl ::= var identifier [: type] (= expr)?
  std::unique_ptr<VarDeclExprAST>
  parseVarDeclaration(bool requiresInitializer) {
    ...
    // This will indicate the type of result of the expression being
    // parsed should be.
    ParsedExprType typeExpr = Type_Unknown;

    // The identifier should be follow by the ':', used to specify the
    // variable's type.
    ...
    if (!lexer.isType(lexer.getCurToken()))
      return parseError<VarDeclExprAST>("$type", "after ':'");

    // Process the tensor type.
    if (lexer.getCurToken() == tok_tensor) {
      typeExpr = Type_Tensor;
      // Eat the 'tensor' keyword.
      lexer.getNextToken();
      ...
      // An empy tensor type. We will figure the size out later.
      if (!type)
        type = std::make_unique<TensorType>();

      // TODO: Check that the variables with tensor type are
      // getting initialized with the [].

    } else if (lexer.getCurToken() == tok_float) {
      typeExpr = Type_Float;
      // Eat the 'float64' keyword.
      lexer.getNextToken();
      type = parseFloatType();

      if (!type)
        return nullptr;
    } else if (lexer.getCurToken() == tok_float64) {
      typeExpr = Type_Float64;
      // Eat the 'float64' keyword.
      lexer.getNextToken();
      type = parseFloat64Type();
      ...
      if (!type)
        return nullptr;
    } else if (lexer.getCurToken() == tok_int64) {
      typeExpr = Type_Int64;
      // Eat the 'int64' keyword.
      lexer.getNextToken();
      type = parseInt64Type();

      if (!type)
        return nullptr;
    } else if (lexer.getCurToken() == tok_char) {
      typeExpr = Type_Char;
      // Eat the 'char' keyword.
      lexer.getNextToken();
      type = parseCharType();
  ...
  std::unique_ptr<ExprASTList> parseBlock() {
    ...
    while (lexer.getCurToken() != '}' && lexer.getCurToken() != tok_eof) {
      if (lexer.getCurToken() == tok_identifier) {
        // A variable declaration or a call.
  ...
  std::unique_ptr<PrototypeAST> parsePrototype() {
    ...
    if (lexer.getCurToken() != ')') {
      do {
        ...
        if (lexer.getCurToken() == tok_identifier) {
          static_cast<TensorType*>(type.get())->setTypeName(std::move(nameOrType));
