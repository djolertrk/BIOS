//===- AST.cpp - Helper for printing out the blang AST ----------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the AST dump for the blang language.
//
//===----------------------------------------------------------------------===//
...
void ASTDumper::dump(NumberExprAST *num) {
  INDENT();
  if (auto numFloat = llvm::dyn_cast<FloatNumberExprAST>(num))
    llvm::errs() << (float)numFloat->getValue() << " " << loc(num) << "\n";
  else if (auto numFloat64 = llvm::dyn_cast<Float64NumberExprAST>(num))
    llvm::errs() << numFloat64->getValue() << " " << loc(num) << "\n";
  else if (auto numInt = llvm::dyn_cast<IntNumberExprAST>(num))
    llvm::errs() << numInt->getValue() << " " << loc(num) << "\n";
  else if (auto numInt64 = llvm::dyn_cast<Int64NumberExprAST>(num))
    llvm::errs() << numInt64->getValue() << " " << loc(num) << "\n";
  else if (auto charNum = llvm::dyn_cast<CharNumberExprAST>(num))
    llvm::errs() << charNum->getValue() << " " << loc(num) << "\n";
  else if (auto stringVar = llvm::dyn_cast<StringExprAST>(num))
    llvm::errs() << stringVar->getValue() << " " << loc(num) << "\n";
  else if (auto numTensor = llvm::dyn_cast<TensorNumberExprAST>(num))
    llvm::errs() << numTensor->getValue() << " " << loc(num) << "\n";
}
...
void printLitHelper(ExprAST *litOrNum) {
  ...
  if (auto num = llvm::dyn_cast<NumberExprAST>(litOrNum)) {
    if (auto numFloat = llvm::dyn_cast<FloatNumberExprAST>(num))
      llvm::errs() << numFloat->getValue() << " " << loc(num) << "\n";
    else if (auto numFloat64 = llvm::dyn_cast<Float64NumberExprAST>(num))
      llvm::errs() << numFloat64->getValue();
    else if (auto numInt = llvm::dyn_cast<IntNumberExprAST>(num))
      llvm::errs() << numInt->getValue() << " " << loc(num) << "\n";
    else if (auto numInt64 = llvm::dyn_cast<Int64NumberExprAST>(num))
      llvm::errs() << numInt64->getValue() << " " << loc(num) << "\n";
...
void ASTDumper::dump(const VarType *type) {
  if (type->getKind() != VarType::Type_Tensor &&
      type->getKind() != VarType::Type_Float &&
      type->getKind() != VarType::Type_Float64 &&
      type->getKind() != VarType::Type_Int &&
      type->getKind() != VarType::Type_Int64 &&
      type->getKind() != VarType::Type_Char &&
      type->getKind() != VarType::Type_String)
    return;

  llvm::errs() << " [type: " << type->getName();

  if (auto tensorType = llvm::dyn_cast<TensorType>(type)) {
    llvm::errs() << "<";
    if (!tensorType->getShapeName().empty())
      llvm::errs() << tensorType->getShapeName();
    else if (!tensorType->getShape().empty())
      mlir::interleaveComma(tensorType->getShape(), llvm::errs());
    llvm::errs() << ">";
  }
