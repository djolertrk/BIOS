//===- Dialect.cpp - blang IR Dialect registration in MLIR ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the dialect for the blang IR: custom type parsing and
...
//===----------------------------------------------------------------------===//
// BlangInlinerInterface
//===----------------------------------------------------------------------===//

/// This class defines the interface for handling inlining with blang
/// operations.
struct BlangInlinerInterface : public DialectInlinerInterface {
  ...
  /// All operations within blang can be inlined.
  ...
  /// Handle the given inlined terminator(blang.return) by replacing it with a new
  /// operation as necessary.
  void handleTerminator(Operation *op,
                        ArrayRef<Value> valuesToRepl) const final {
    // Only "blang.return" needs to be handled here.
...
blangDialect::blangDialect(mlir::MLIRContext *ctx) : mlir::Dialect("blang", ctx) {
  addOperations<
#define GET_OP_LIST
#include "blang/mlir/Ops.cpp.inc"
      >();
  addInterfaces<BlangInlinerInterface>();
  addTypes<StructType>();
}

mlir::Operation *blangDialect::materializeConstant(mlir::OpBuilder &builder,
...
/// Build a tensor constant operation.
...
/// Build a float32 constant operation.
void ConstantFloat32Op::build(mlir::Builder *builder, mlir::OperationState &state,
                       float value) {
  auto dataType = builder->getF32Type();
  auto dataAttribute = FloatAttr::get(dataType, value);
  ConstantFloat32Op::build(builder, state, dataAttribute);
}

/// Build a float64 constant operation.
void ConstantFloat64Op::build(mlir::Builder *builder, mlir::OperationState &state,
                       double value) {
  auto dataType = builder->getF64Type();
  auto dataAttribute = FloatAttr::get(dataType, value);
  ConstantFloat64Op::build(builder, state, dataAttribute);
}

/// Build a int constant operation.
void ConstantIntOp::build(mlir::Builder *builder, mlir::OperationState &state,
                          int value) {
  auto dataType = builder->getIntegerType(32);
  auto dataAttribute = IntegerAttr::get(dataType, value);
  ConstantIntOp::build(builder, state, dataAttribute);
}

/// Build a int constant operation.
void ConstantInt64Op::build(mlir::Builder *builder, mlir::OperationState &state,
                          int64_t value) {
  auto dataType = builder->getIntegerType(64);
  auto dataAttribute = IntegerAttr::get(dataType, value);
  ConstantInt64Op::build(builder, state, dataAttribute);
}

/// Build a char constant operation.
void ConstantCharOp::build(mlir::Builder *builder, mlir::OperationState &state,
                          char value) {
  auto dataType = builder->getIntegerType(8);
  auto dataAttribute = IntegerAttr::get(dataType, value);
  ConstantCharOp::build(builder, state, dataAttribute);
}

/// Build a char constant operation.
void ConstantStringOp::build(mlir::Builder *builder, mlir::OperationState &state,
                          std::string value) {
  auto dataAttribute = builder->getStringAttr(value);
  ConstantStringOp::build(builder, state, dataAttribute);
}
...
/// Verify the constant_float32 operation.
static mlir::LogicalResult verify(ConstantFloat32Op op) {
  /// TODO: Implement this.
  return mlir::success();
}

/// Verify the constant_float operation.
static mlir::LogicalResult verify(ConstantFloat64Op op) {
  /// TODO: Implement this.
  return mlir::success();
}

/// Verify the constant_int operation.
static mlir::LogicalResult verify(ConstantIntOp op) {
  /// TODO: Implement this.
  return mlir::success();
}

/// Verify the constant_int64 operation.
static mlir::LogicalResult verify(ConstantInt64Op op) {
  /// TODO: Implement this.
  return mlir::success();
}

/// Verify the constant_int64 operation.
static mlir::LogicalResult verify(ConstantCharOp op) {
  /// TODO: Implement this.
  return mlir::success();
}

/// Verify the constant_string operation.
static mlir::LogicalResult verify(ConstantStringOp op) {
  /// TODO: Implement this.
  return mlir::success();
}
...
namespace blang {
namespace detail {
/// This class represents the internal storage of the blang `StructType`.
...
StructType StructType::get(llvm::ArrayRef<mlir::Type> elementTypes) {
  ...
  return Base::get(ctx, BlangTypes::Struct, elementTypes);
}
...
mlir::Type blangDialect::parseType(mlir::DialectAsmParser &parser) const {
...
void blangDialect::printType(mlir::Type type,
                           mlir::DialectAsmPrinter &printer) const {
  // Currently the only blang type is a struct type.
